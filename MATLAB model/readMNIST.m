...
function [imgs labels] = readMNIST(imgFile, labelFile, readDigits, offset)
    ...
    % imgs = trimDigits(imgs, 4); % Commented to keep 28x28 images
    imgs = expandDigits(imgs, 2);
    imgs = normalizePixValue(imgs);
    %[avg num stddev] = getDigitStats(imgs, labels);
    
end
function digits = expandDigits(digitsIn, border)
    dSize = size(digitsIn);
    digits = zeros([dSize(1)+(border*2) dSize(2)+(border*2) dSize(3)]);
    for i=1:dSize(3)
        digits(border+1:dSize(1)+border, border+1:dSize(2)+border, i) = digitsIn(:,:,i);
    end
end
